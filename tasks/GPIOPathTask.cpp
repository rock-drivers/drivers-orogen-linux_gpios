...
GPIOPathTask::GPIOPathTask(string const& name)
    : GPIOPathTaskBase(name)
{
}

GPIOPathTask::GPIOPathTask(string const& name, RTT::ExecutionEngine* engine)
    : GPIOPathTaskBase(name, engine)
{
}

GPIOPathTask::~GPIOPathTask()
{
}

/// The following lines are template definitions for the various state machine
// hooks defined by Orocos::RTT. See GPIOPathTask.hpp for more detailed
// documentation about them.

bool GPIOPathTask::configureHook()
{
    if (!GPIOPathTaskBase::configureHook())
        return false;
...
bool GPIOPathTask::startHook()
{
    if (!GPIOPathTaskBase::startHook())
        return false;
...
void GPIOPathTask::updateHook()
{
    ...
    GPIOPathTaskBase::updateHook();
}

bool GPIOPathTask::handleWriteSide()
{
    auto now = base::Time::now();
...
void GPIOPathTask::writeDefaults()
{
    if (m_write_configuration.defaults.empty()) {
        return;
    }
...
void GPIOPathTask::errorHook()
{
    GPIOPathTaskBase::errorHook();
}
void GPIOPathTask::stopHook()
{
    GPIOPathTaskBase::stopHook();
}
void GPIOPathTask::cleanupHook()
{
    closeAll();
    GPIOPathTaskBase::cleanupHook();
}
    ...
    int mode)
{
    ...
    for (string gpio_path : gpio_paths) {
        string sysfs_path = gpio_path + "/value";
...
void GPIOPathTask::writeGPIO(int fd, bool value)
{
    char buf = value ? '1' : '0';
...
bool GPIOPathTask::readGPIO(int fd)
{
    char buf;
